...
{
    private static double deltat = 60; //TODO define 30 seems to bring decent results...

    public static Quaternion MadgwickQuaternionUpdate(double ax, double ay, double az, double gx, double gy, double gz, double mx, double my, double mz)
    {
        double GyroMeasError = Math.PI * (40.0f / 180.0f);
        ...
        norm = Math.Sqrt(d);
        if (norm == 0.0f) return new Quaternion(); // handle NaN
        norm = 1.0f / norm;
        ax *= norm;
        ay *= norm;
        az *= norm;
        ...
        return new Quaternion((float)q[1], (float)q[3], (float)q[2], (float)q[0]);
    }
}
